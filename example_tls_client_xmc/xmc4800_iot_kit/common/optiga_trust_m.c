...
#define LENGTH_OPTIGA_CERT          920

extern uint8_t TrustM_device_cert[LENGTH_OPTIGA_CERT];
...
void vTrustMTaskCallbackHandler( void * pvParameters )
{
	...
	if ( xSemaphoreTake(xTrustMSemaphoreHandle, xTrustMSemaphoreWaitTicks) == pdTRUE )
	{
		...
		TrustM_Cert_Len = LENGTH_OPTIGA_CERT;

		
		optiga_lib_status_t read_chip_cert_status = read_chip_cert(OID, TrustM_device_cert, (uint16_t*)&TrustM_Cert_Len);
